...
public class ContainerGroundTrap extends Container {
    ...
    public ContainerGroundTrap(InventoryPlayer inv, TileEntityGroundTrap TileEntityGroundTrap) {
        ...
        for (int x = 0; x < 6; ++x) {
            for (int y = 0; y < 3; ++y) {
                addSlotToContainer(new SlotPamResult(TileEntityGroundTrap, y + x * 3, 62 + 18 * x, 17 + 18 * y));
            }
        }

        addSlotToContainer(new SlotPamGroundTrap(TileEntityGroundTrap, 18, 26, 35));
    ...
    public void detectAndSendChanges() {
        ...
        for (IContainerListener listener : listeners) {
            ...
            if (lastBeeRunTime != groundtrap.currentBeeRunTime) {
                listener.sendProgressBarUpdate(this, 3, groundtrap.currentBeeRunTime);
            }
        }
    ...
    public boolean canInteractWith(EntityPlayer playerIn) {
        return groundtrap.isUseableByPlayer(playerIn);
    }
    ...
    /*     */       
    /*  88 */       if (slotIndex >= 19) {
    /*  89 */         if ((slotStack.getItem() == ItemRegistry.fruitbaitItem) || (slotStack.getItem() == ItemRegistry.grainbaitItem) || (slotStack.getItem() == ItemRegistry.veggiebaitItem)) {
