...
public class GuiShippingBin extends GuiContainer {
    ...
    protected void actionPerformed(GuiButton guibutton) {
        ...
        if (guibutton.id == 2) {
            ItemStack buySlot = this.tileEntityShippingBin.getStackInSlot(0);
            if (buySlot != null) {
                ...
                if (buySlot.getItem() == data.getCurrency().getItem()) {
                    if (buySlot.getItemDamage() == data.getCurrency().getItemDamage()) {
                        ...
                        if (buySlot.stackSize == price) {
                        	System.out.println("Stupid ShippingBin");
                            PacketHandler.network.sendToServer(new MessageShippingBinBuy(this.itemNum, this.tileEntityShippingBin.getPos().getX(), this.tileEntityShippingBin.getPos().getY(), this.tileEntityShippingBin.getPos().getZ(), true));
                        } else if (buySlot.stackSize > price && buySlot.stackSize > 1) {
                            PacketHandler.network.sendToServer(new MessageShippingBinBuy(this.itemNum, this.tileEntityShippingBin.getPos().getX(), this.tileEntityShippingBin.getPos().getY(), this.tileEntityShippingBin.getPos().getZ(), false));
                        }
    ...
    public void onGuiClosed() {
        PacketHandler.network.sendToServer(new MessageShippingBinClosed(this.tileEntityShippingBin.getPos().getX(), this.tileEntityShippingBin.getPos().getY(), this.tileEntityShippingBin.getPos().getZ()));
        super.onGuiClosed();
    }
